...
##########################################################################################################  DEBERIA COMPROBAR SI ES TIENEN CROMOSOMA X O CROMOSOMA 23 LOS INPUTS?

plink --bfile $data --freq --out $data # Generate freq file
perl HRC-1000G-check-bim.pl -b $data.bim -f $data.frq -r /home/isglobal.lan/itolosana/homews/reference_panels/1000GP_Phase3_combined.legend -g # Run script following the steps described in: http://www.well.ox.ac.uk/~wrayner/tools/
...
if [[ $format == plink ]] || [[ $format == PLINK ]] # If the dataformat indicated is plink (.bed, .bim, .fam)
then
