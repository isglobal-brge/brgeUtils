...
byregion_17 <- readVcf("/scratch/itolosana/function_impute/17_subset_imputed_byregion_final.vcf.gz", "hg19")
whole_17 <- readVcf("/scratch/itolosana/function_impute/17_subset_imputed_final.vcf.gz", "hg19")
```
...
## DS values
```{r echo=FALSE}
DS_byregion <- geno(byregion_17)$DS
DS_whole <- geno(whole_17)$DS
```
...
## R^2^
```{r echo=FALSE}
rsq <- c(info(byregion_17)$R2, info(whole_17)$R2)
ismich <- rep("ByRegion", length(info(byregion_17)$R2))
ismich <- c(ismich, rep("WholeChr", length(info(whole_17)$R2)))
...
par(mfrow=c(1,2))
HIST_R2_BYREGION <- hist(info(byregion_17)$R2, breaks=seq(0, 1, by=0.05), 
                         main="ByRegion", xlab="Imputation R2")
HIST_R2_WHOLE <- hist(info(whole_17)$R2, breaks=seq(0, 1, by=0.05), 
...
gen_byregion <- genotypeToSnpMatrix(byregion_17)
matrix_gen_byregion <- t(as(gen_byregion$genotype, "character"))

gen_whole <- genotypeToSnpMatrix(whole_17)
...
data("Refs")
ref <- Refs$inv17q21.31
data("SNPsR2")
R2s <- SNPsR2$inv17q21.31
data("hetRefs")
hRefs <- hetRefs$inv17q21.31


byregion_inv_17 <- scoreInvHap(SNPlist = byregion_17, SNPsR2 = R2s, hetRefs = hRefs, Refs = ref, imputed = TRUE, verbose = TRUE, BPPARAM = MulticoreParam(10))

whole_inv_17 <- scoreInvHap(SNPlist = whole_17, SNPsR2 = R2s, hetRefs = hRefs, Refs = ref, imputed = TRUE, verbose = TRUE, BPPARAM = MulticoreParam(10))
```
...
```{r echo=FALSE}
scoreinvhap_table <- table(ByRegion = classification(byregion_inv_17)[sort(names(classification(byregion_inv_17)))], 
      WholeChr = classification(whole_inv_17)[sort(names(classification(whole_inv_17)))])
```
...
par(mfrow=c(1,2))
hist(maxscores(byregion_inv_17), breaks=seq(0, 1, by=0.05), main="ByRegion", xlab="Maxscores")
hist(maxscores(whole_inv_17), breaks=seq(0, 1, by=0.05), main="WholeChr", xlab="Maxscores")
```

```{r echo=FALSE}
# Calculate scores correlation by individuals
score_corr <- mapply(cor, as.data.frame(scores(byregion_inv_17)[sort(rownames(scores(byregion_inv_17))),]),
                          as.data.frame(scores(whole_inv_17)[sort(rownames(scores(whole_inv_17))),]))
```
...
par(mfrow=c(1,2))
hist(diffscores(byregion_inv_17), breaks=seq(0, 1, by=0.05), 
     main="ByRegioin", xlab="Difference score")
hist(diffscores(whole_inv_17), breaks=seq(0, 1, by=0.05), 
...
mean(numSNPs(byregion_inv_17))
mean(numSNPs(whole_inv_17))
```

```{r}
# Number of samples in both imputation methods before and after QC filtering
length(classification(byregion_inv_17))
length(classification(byregion_inv_17, minDiff = 0.1, callRate = 0.9))
length(classification(byregion_inv_17, minDiff = 0.1, callRate = 0.9))/
  length(classification(byregion_inv_17))
length(classification(whole_inv_17))
length(classification(whole_inv_17, minDiff = 0.1, callRate = 0.9))
length(classification(whole_inv_17, minDiff = 0.1, callRate = 0.9))/
  length(classification(whole_inv_17))
```
...
```{r}
# Steps to compare the byregion_17 with the nofilter_minimac_17
whole_17 <- nofilter_minimac_17

whole_17 <- whole_17[,colnames(byregion_17)]

noinv17 <- classification(nofilter_minimac_inv_17)[names(classification(byregion_inv_17))]

table(ByRegion = classification(byregion_inv_17)[sort(names(classification(byregion_inv_17)))],
