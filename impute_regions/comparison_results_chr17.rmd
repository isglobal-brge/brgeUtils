---
title: "Comparison: Michigan Imputation Server / Shapeit+Minimac3 (Chromosome 17)"
...
load(file = "michigan_chr17.rda")
load(file = "michigan_invclust_chr17.rda")
load(file = "michigan_scoreinvhap_chr17.rda")
load(file = "minimac_chr17.rda")
load(file = "minimac_invclust_chr17.rda")
load(file = "minimac_scoreinvhap_chr17.rda")
#load(file = "nofilter_michigan_17.rda")
#load(file = "nofilter_michigan_inv_17.rda")
#load(file = "nofilter_michigan_invclust_17.rda")
load(file = "nofilter_minimac_17.rda")
load(file = "nofilter_minimac_inv_17.rda")
#load(file = "nofilter_minimac_invclust_17.rda")
```
...
## DS values
```{r echo=FALSE}
DS_michigan <- geno(michigan_chr17)$DS
DS_minimac <- geno(minimac_chr17)$DS
```
...
## R^2^
```{r echo=FALSE}
rsq <- c(info(michigan_chr17)$R2, info(minimac_chr17)$R2)
ismich <- rep("Michigan", length(info(michigan_chr17)$R2))
ismich <- c(ismich, rep("Minimac", length(info(minimac_chr17)$R2)))
...
gen_michigan <- genotypeToSnpMatrix(michigan_chr17)
matrix_gen_michigan <- t(as(gen_michigan$genotype, "character"))

gen_minimac <- genotypeToSnpMatrix(minimac_chr17)
...
```{r echo=FALSE}
scoreinvhap_table <- table(Michigan = classification(michigan_inv_chr17)[sort(names(classification(michigan_inv_chr17)))], 
      Minimac = classification(minimac_inv_chr17)[sort(names(classification(minimac_inv_chr17)))])
```
...
score_corr <- mapply(cor, as.data.frame(scores(michigan_inv_chr17)[sort(rownames(scores(michigan_inv_chr17))),]),
                          as.data.frame(scores(minimac_inv_chr17)[sort(rownames(scores(minimac_inv_chr17))),]))
```
...
```{r echo=FALSE}
scoreinvhap_table_filt <- table(No_filtered = classification(nofilter_minimac_inv_17)[sort(names(classification(nofilter_minimac_inv_17)))], 
      Filtered = classification(minimac_inv_chr17)[sort(names(classification(minimac_inv_chr17)))])
```
