...
byregion_8 <- readVcf("/scratch/itolosana/function_impute/8_subset_imputed_byregion_final.vcf.gz", "hg19")
whole_8 <- readVcf("/scratch/itolosana/function_impute/8_subset_imputed_final.vcf.gz", "hg19")
```
...
## DS values
```{r echo=FALSE}
DS_byregion <- geno(byregion_8)$DS
DS_whole <- geno(whole_8)$DS
```
...
## R^2^
```{r echo=FALSE}
rsq <- c(info(byregion_8)$R2, info(whole_8)$R2)
ismich <- rep("ByRegion", length(info(byregion_8)$R2))
ismich <- c(ismich, rep("WholeChr", length(info(whole_8)$R2)))
...
par(mfrow=c(1,2))
HIST_R2_BYREGION <- hist(info(byregion_8)$R2, breaks=seq(0, 1, by=0.05), 
                         main="ByRegion", xlab="Imputation R2")
HIST_R2_WHOLE <- hist(info(whole_8)$R2, breaks=seq(0, 1, by=0.05), 
...
gen_byregion <- genotypeToSnpMatrix(byregion_8)
matrix_gen_byregion <- t(as(gen_byregion$genotype, "character"))

gen_whole <- genotypeToSnpMatrix(whole_8)
...
byregion_inv_8 <- scoreInvHap(SNPlist = byregion_8, SNPsR2 = R2s, hetRefs = hRefs, Refs = ref, imputed = TRUE, verbose = TRUE, BPPARAM = MulticoreParam(10))

whole_inv_8 <- scoreInvHap(SNPlist = whole_8, SNPsR2 = R2s, hetRefs = hRefs, Refs = ref, imputed = TRUE, verbose = TRUE, BPPARAM = MulticoreParam(10))
```
...
```{r echo=FALSE}
scoreinvhap_table <- table(ByRegion = classification(byregion_inv_8)[sort(names(classification(byregion_inv_8)))], 
      WholeChr = classification(whole_inv_8)[sort(names(classification(whole_inv_8)))])
```
...
par(mfrow=c(1,2))
hist(maxscores(byregion_inv_8), breaks=seq(0, 1, by=0.05), main="ByRegion", xlab="Maxscores")
hist(maxscores(whole_inv_8), breaks=seq(0, 1, by=0.05), main="WholeChr", xlab="Maxscores")
```

```{r echo=FALSE}
# Calculate scores correlation by individuals
score_corr <- mapply(cor, as.data.frame(scores(byregion_inv_8)[sort(rownames(scores(byregion_inv_8))),]),
                          as.data.frame(scores(whole_inv_8)[sort(rownames(scores(whole_inv_8))),]))
```
...
par(mfrow=c(1,2))
hist(diffscores(byregion_inv_8), breaks=seq(0, 1, by=0.05), 
     main="ByRegioin", xlab="Difference score")
hist(diffscores(whole_inv_8), breaks=seq(0, 1, by=0.05), 
...
mean(numSNPs(byregion_inv_8))
mean(numSNPs(whole_inv_8))
```

```{r}
# Number of samples in both imputation methods before and after QC filtering
length(classification(byregion_inv_8))
length(classification(byregion_inv_8, minDiff = 0.1, callRate = 0.9))
length(classification(byregion_inv_8, minDiff = 0.1, callRate = 0.9))/
  length(classification(byregion_inv_8))
length(classification(whole_inv_8))
length(classification(whole_inv_8, minDiff = 0.1, callRate = 0.9))
length(classification(whole_inv_8, minDiff = 0.1, callRate = 0.9))/
  length(classification(whole_inv_8))
```
