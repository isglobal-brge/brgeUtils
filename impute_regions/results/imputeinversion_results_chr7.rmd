...
byregion_7 <- readVcf("/scratch/itolosana/function_impute/7_subset_imputed_byregion_final.vcf.gz", "hg19")
whole_7 <- readVcf("/scratch/itolosana/function_impute/7_subset_imputed_final.vcf.gz", "hg19")
```
...
## DS values
```{r echo=FALSE}
DS_byregion <- geno(byregion_7)$DS
...
## R^2^
```{r echo=FALSE}
rsq <- c(info(byregion_7)$R2, info(whole_7)$R2)
ismich <- rep("ByRegion", length(info(byregion_7)$R2))
...
par(mfrow=c(1,2))
HIST_R2_BYREGION <- hist(info(byregion_7)$R2, breaks=seq(0, 1, by=0.05), 
...
gen_byregion <- genotypeToSnpMatrix(byregion_7)
...
byregion_inv_7 <- scoreInvHap(SNPlist = byregion_7, SNPsR2 = R2s, hetRefs = hRefs, Refs = ref, imputed = TRUE, verbose = TRUE, BPPARAM = MulticoreParam(10))
...
```{r echo=FALSE}
scoreinvhap_table <- table(ByRegion = classification(byregion_inv_7)[sort(names(classification(byregion_inv_7)))], 
...
par(mfrow=c(1,2))
hist(maxscores(byregion_inv_7), breaks=seq(0, 1, by=0.05), main="ByRegion", xlab="Maxscores")
...
score_corr <- mapply(cor, as.data.frame(scores(byregion_inv_7)[sort(rownames(scores(byregion_inv_7))),]),
...
par(mfrow=c(1,2))
hist(diffscores(byregion_inv_7), breaks=seq(0, 1, by=0.05), 
...
mean(numSNPs(byregion_inv_7))
mean(numSNPs(whole_inv_7))
```

```{r}
# Number of samples in both imputation methods before and after QC filtering
length(classification(byregion_inv_7))
length(classification(byregion_inv_7, minDiff = 0.1, callRate = 0.9))
length(classification(byregion_inv_7, minDiff = 0.1, callRate = 0.9))/
  length(classification(byregion_inv_7))
