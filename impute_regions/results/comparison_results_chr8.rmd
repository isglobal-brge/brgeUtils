---
title: "Comparison: Michigan Imputation Server / Shapeit+Minimac3 (Chromosome 8)"
...
load(file = "michigan_chr8.rda")
load(file = "michigan_invclust_chr8.rda")
load(file = "michigan_scoreinvhap_chr8.rda")
load(file = "minimac_chr8.rda")
load(file = "minimac_invclust_chr8.rda")
load(file = "minimac_scoreinvhap_chr8.rda")
#load(file = "nofilter_michigan_8.rda")
#load(file = "nofilter_michigan_inv_8.rda")
#load(file = "nofilter_michigan_invclust_8.rda")
load(file = "nofilter_minimac_8.rda")
load(file = "nofilter_minimac_inv_8.rda")
#load(file = "nofilter_minimac_invclust_8.rda")
```
...
## DS values
```{r echo=FALSE}
DS_michigan <- geno(michigan_chr8)$DS
DS_minimac <- geno(minimac_chr8)$DS
```
...
## R^2^
```{r echo=FALSE}
rsq <- c(info(michigan_chr8)$R2, info(minimac_chr8)$R2)
ismich <- rep("Michigan", length(info(michigan_chr8)$R2))
ismich <- c(ismich, rep("Minimac", length(info(minimac_chr8)$R2)))
...
gen_michigan <- genotypeToSnpMatrix(michigan_chr8)
matrix_gen_michigan <- t(as(gen_michigan$genotype, "character"))

gen_minimac <- genotypeToSnpMatrix(minimac_chr8)
...
```{r echo=FALSE}
scoreinvhap_table <- table(Michigan = classification(michigan_inv_chr8)[sort(names(classification(michigan_inv_chr8)))], 
      Minimac = classification(minimac_inv_chr8)[sort(names(classification(minimac_inv_chr8)))])
```
...
score_corr <- mapply(cor, as.data.frame(scores(michigan_inv_chr8)[sort(rownames(scores(michigan_inv_chr8))),]),
                          as.data.frame(scores(minimac_inv_chr8)[sort(rownames(scores(minimac_inv_chr8))),]))
```
...
```{r echo=FALSE}
scoreinvhap_table_filt <- table(No_filtered = classification(nofilter_minimac_inv_8)[sort(names(classification(nofilter_minimac_inv_8)))], 
      Filtered = classification(minimac_inv_chr8)[sort(names(classification(minimac_inv_chr8)))])
```
